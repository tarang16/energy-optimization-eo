...
import EnpiDollar, { EnpiDollarChartConfig } from './EnpiDollar'
...
describe('EnpiDollar component', () => {
  ...
  it("shows 'No Data found' when empty data is passed", async () => {
    render(<EnpiDollar dateRange={mockDateRange} energyData={[]} />)
  ...
  it('renders ComboChart with full config when modal is closed', async () => {
    ...
    render(<EnpiDollar dateRange={mockDateRange} energyData={mockData} />)
    expect(await screen.findByTestId('combo-chart')).toBeInTheDocument()
  })
})

describe('Chart EnpiDollarChartConfig adapterFn', () => {
  beforeAll(() => {
    if (!Array.prototype.toSorted) {
      Array.prototype.toSorted = function (compareFn) {
        return [...this].sort(compareFn)
  ...
  it('returns primary_blue when airEnpi <= airEnpiTarget', () => {
    ...
    const adapterFn = EnpiDollarChartConfig.series[0].column.template.adapterFn
