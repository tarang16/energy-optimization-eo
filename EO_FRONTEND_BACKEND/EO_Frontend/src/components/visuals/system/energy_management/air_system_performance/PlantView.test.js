...
import PlantView, { plantViewChartConfig } from './PlantView'
...
describe('Chart plantViewChartConfig adapterFn', () => {
  it('returns primary_blue when airEnpi <= airEnpiTarget', () => {
    ...
    const adapterFn = plantViewChartConfig.series[1].column.template.adapterFn
  ...
  it('returns primary_blue when airSpecificEnpi <= airSpecificEnpiTarget', () => {
    ...
    const adapterFn = plantViewChartConfig.series[3].column.template.adapterFn
    const result = adapterFn(dataItem)
  ...
  it('returns primary_gray_2 for AIR ENPI Target series adapterFn', () => {
    ...
    const adapterFn = plantViewChartConfig.series[0].column.template.adapterFn
    const result = adapterFn(dataItem)
  ...
  it('returns primary_gray_2 for SPECIFIC AIR ENPI Target series adapterFn', () => {
    const dataItem = { dataContext: {} }
    const adapterFn = plantViewChartConfig.series[2].column.template.adapterFn
    const result = adapterFn(dataItem)
    expect(result).toBe('#939598') // primary_gray_2
  })
})
