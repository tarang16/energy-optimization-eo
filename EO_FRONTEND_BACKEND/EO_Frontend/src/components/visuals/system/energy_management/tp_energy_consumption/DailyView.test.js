...
// --------------------------------------------------
describe('DailyView Component - Coverage', () => {
  const defaultProps = {
  ...
  test('renders ComboChart when data present and covers adapterFn branches, and can expand to show modal', async () => {
    ...
    const mockData = [
      {
        groupByCol: '2025-05-01',
        energyConsumed: 12,
        energyConsumedTarget: 15,
        specificEnergyConsumption: 6,
        specificEnergyConsumptionTarget: 10,
        energyIntensity: 4,
      },
    ]
    ...
    expect(extractValueBeforeParens).toHaveBeenCalledWith('Daily Header (info)')
  ...
  test('handles unexpected header string without throwing', async () => {
    ...
    getEnergyConsumedSpecificEnergy.mockResolvedValue({
      data: [{ groupByCol: '2025-05-02', energyConsumed: 8 }],
    })
    ...
    extractValueBeforeParens.mockReturnValue('NoParenDailyHeader')
    ...
    // Ensure extractValueBeforeParens was still called with the original header
    expect(extractValueBeforeParens).toHaveBeenCalledWith('Daily Header (info)')
  })
})
