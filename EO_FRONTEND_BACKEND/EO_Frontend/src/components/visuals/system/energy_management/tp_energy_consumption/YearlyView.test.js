        ...
        <button
          ...
          onClick={() =>
            props.setExpandModal({
              id: 'yAxis2',
              axisHeader: { text: 'Yearly Header (info)' },
            })
          }
        >
          Expand
...
// --------------------------------------------------
describe('YearlyView Component - Coverage', () => {
  const defaultProps = {
  ...
  test('renders ComboChart when data present and covers adapterFn branches, and can expand to show modal', async () => {
    ...
    expect(extractValueBeforeParens).toHaveBeenCalledWith(
      'Yearly Header (info)',
    )
    ...
    // Modal still contains a ComboChart
  ...
  test('handles unexpected header string without throwing', async () => {
    ...
    // Override extractValueBeforeParens to return full text
    extractValueBeforeParens.mockReturnValue('NoParenYearlyHeader')
    ...
    // Ensure extractValueBeforeParens was still called
    expect(extractValueBeforeParens).toHaveBeenCalledWith(
      'Yearly Header (info)',
    )
  })
